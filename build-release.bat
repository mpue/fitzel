...
echo Baue sandbox + player + shadercheck + audiocheck + citycheck + iconcheck + skycheck + fogcheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck citycheck iconcheck skycheck fogcheck fpak
if %errorlevel% neq 0 (
