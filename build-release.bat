...
echo Baue sandbox + player + shadercheck + audiocheck + autosavecheck + meshpaintcheck + modelcheck + importcheck + softcheck + viewcheck + citycheck + shotcheck + iconcheck + skycheck + fogcheck + pathcheck + capturecheck + rivercheck + weathercheck + roadcheck + sculptcheck + selcheck + gpucheck + tracecheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck autosavecheck meshpaintcheck modelcheck importcheck softcheck viewcheck citycheck shotcheck iconcheck skycheck fogcheck pathcheck capturecheck rivercheck weathercheck roadcheck sculptcheck selcheck gpucheck tracecheck fpak
if %errorlevel% neq 0 (
