...
REM  IDE gegen eine spaeter deinstallierte Insiders-Installation erzeugt),
...
echo Baue sandbox + player + shadercheck + audiocheck + autosavecheck + meshpaintcheck + modelcheck + synthcheck + synthplaycheck + importcheck + softcheck + viewcheck + citycheck + shotcheck + iconcheck + skycheck + fogcheck + pathcheck + capturecheck + rivercheck + weathercheck + roadcheck + sculptcheck + selcheck + animcheck + graphcheck + gpucheck + tracecheck + grasscheck + camcheck + gridcheck + prefabeditcheck + splineplacecheck + levelcheck + mixercheck + collidecheck + streamcheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck synthcheck synthplaycheck autosavecheck meshpaintcheck modelcheck importcheck softcheck viewcheck citycheck shotcheck iconcheck skycheck fogcheck pathcheck capturecheck rivercheck weathercheck roadcheck sculptcheck selcheck animcheck graphcheck gpucheck tracecheck grasscheck camcheck gridcheck prefabeditcheck splineplacecheck levelcheck mixercheck collidecheck streamcheck fpak
if %errorlevel% neq 0 (
