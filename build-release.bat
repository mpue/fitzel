...
echo Baue sandbox + player + shadercheck + audiocheck + autosavecheck + meshpaintcheck + softcheck + viewcheck + citycheck + shotcheck + iconcheck + skycheck + fogcheck + pathcheck + capturecheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck autosavecheck meshpaintcheck softcheck viewcheck citycheck shotcheck iconcheck skycheck fogcheck pathcheck capturecheck fpak
if %errorlevel% neq 0 (
