...
echo Baue sandbox + player + shadercheck + audiocheck + citycheck + iconcheck + skycheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck citycheck iconcheck skycheck fpak
if %errorlevel% neq 0 (
