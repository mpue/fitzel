...
echo Baue sandbox + player + shadercheck + audiocheck + citycheck + iconcheck + skycheck + cloudcheck + fpak ^(Release^)...
"%CMAKE%" --build build\release --target sandbox player shadercheck audiocheck citycheck iconcheck skycheck cloudcheck fpak
if %errorlevel% neq 0 (
