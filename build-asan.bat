@echo off
REM ============================================================================
REM  Fitzel - ASan-Build (Ninja, Release + AddressSanitizer)
REM
REM  cmake und cl sind auf diesem Rechner nicht in der PATH-Variable. Dieses
REM  Skript laedt die VS18-Toolchain (vcvars64) und baut denselben Satz Targets
REM  wie build-release.bat, aber in build\asan und mit /fsanitize=address auf
REM  unserem eigenen Code. Ergebnis: build\asan\bin\sandbox.exe
REM
REM  Wofuer: ein Use-after-free stuerzt nicht dort ab, wo der Fehler gemacht
REM  wurde, sondern dort, wo der Allokator den Block spaeter wieder ausgibt.
REM  Unter ASan bricht es beim Zugriff selbst ab und nennt Allokation und
REM  Freigabe dazu. Kostet zwei- bis dreimal Laufzeit, also ein eigener Baum
REM  neben dem Release-Build, kein Ersatz dafuer.
REM
REM  Pruefen damit:  check-all.bat --asan
REM
REM  Aufruf einfach per Doppelklick oder in der Konsole:  build-asan.bat
REM ============================================================================
...
set "NEEDCONFIG=0"
if not exist "build\asan\build.ninja" set "NEEDCONFIG=1"
if exist "build\asan\CMakeCache.txt" (
    findstr /C:"18/Insiders/" "build\asan\CMakeCache.txt" >nul 2>&1 || (
...
if "%NEEDCONFIG%"=="1" (
    echo Konfiguriere ASan-Preset...
    "%CMAKE%" --preset asan -DCMAKE_C_COMPILER="%VSCL%" -DCMAKE_CXX_COMPILER="%VSCL%" -DCMAKE_MAKE_PROGRAM="%NINJA%"
    if %errorlevel% neq 0 (
...
set "PYEXE="
for /f "tokens=2 delims==" %%P in ('findstr /B /C:"_Python_EXECUTABLE:INTERNAL=" "build\asan\CMakeCache.txt" 2^>nul') do set "PYEXE=%%P"
...
echo Baue ASan: sandbox + player + shadercheck + audiocheck + autosavecheck + meshpaintcheck + viewcheck + citycheck + shotcheck + iconcheck + skycheck + fogcheck + pathcheck + capturecheck + fpak ^(Release+ASan^)...
"%CMAKE%" --build build\asan --target sandbox player shadercheck audiocheck autosavecheck meshpaintcheck viewcheck citycheck shotcheck iconcheck skycheck fogcheck pathcheck capturecheck fpak
if %errorlevel% neq 0 (
...
echo.
echo Fertig:  build\asan\bin\sandbox.exe  ^(Editor unter ASan^)
echo.
echo Pruefen:  check-all.bat --asan
endlocal
