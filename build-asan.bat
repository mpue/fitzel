...
echo Baue ASan: sandbox + player + shadercheck + audiocheck + autosavecheck + meshpaintcheck + citycheck + shotcheck + iconcheck + skycheck + fogcheck + pathcheck + capturecheck + fpak ^(Release+ASan^)...
"%CMAKE%" --build build\asan --target sandbox player shadercheck audiocheck autosavecheck meshpaintcheck citycheck shotcheck iconcheck skycheck fogcheck pathcheck capturecheck fpak
if %errorlevel% neq 0 (
